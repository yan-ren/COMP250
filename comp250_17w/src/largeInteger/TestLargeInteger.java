...
class LargeInteger {
    ...
    public LargeInteger iterativeAddition(LargeInteger other) {
        ...
        counter = new LargeInteger(this.digits, 0, this.digits.length);
    	LargeInteger i = new LargeInteger("1");
    	LargeInteger zero = new LargeInteger("0");
    	
    	while(!counter.equals(zero)){
    		sum = sum.add(other);
    		counter = counter.subtract(i);
    	}
    	
    	return sum; // Remove this from your code.
    ...
    public LargeInteger standardMultiplication(LargeInteger other) {
    	...
    	LargeInteger lMultiplier, sMultiplier;
    	LargeInteger sum = new LargeInteger("0");
    	if(this.digits.length < other.digits.length){
    		sMultiplier = new LargeInteger(this.digits, 0, this.digits.length);
    		lMultiplier = new LargeInteger(other.digits, 0, this.digits.length);
    	}else{
    		lMultiplier = new LargeInteger(this.digits, 0, this.digits.length);
    		sMultiplier = new LargeInteger(other.digits, 0, this.digits.length);
    	}
    	int counter = 0;
    	for(int i = sMultiplier.digits.length -1; i >= 0; i--){
    		int carry = 0;
    		LargeInteger tmpAdd = new LargeInteger(lMultiplier.digits.length +1);
    		for(int j = lMultiplier.digits.length - 1; j >=0; j--){
    			int c = sMultiplier.digits[i] * lMultiplier.digits[j] + carry;
    			tmpAdd.digits[j+1] = (byte)(c%10);
    			carry = c/10;
    		}
    		tmpAdd.digits[0] = (byte) carry;
    		tmpAdd = tmpAdd.removeLeadingZeros();
    		sum = sum.add(tmpAdd).shiftLeft(counter);
    		counter++;
    	}
...
//        System.out.println(b + " * " + a + " = " + b.recursiveMultiplication( a ) );
    LargeInteger a = new LargeInteger("8");
    LargeInteger b = new LargeInteger("4");
    System.out.println(a + " * " + b + " = " + a.standardMultiplication(b));
    }
}
